...
const { updateGtfsHealth } = require('../../routes/health');
...
const updateGtfsRt = async () => {
  try {
    console.log('🔄 Updating GTFS realtime data with native timeout support...');
    
    // Usar directamente la función nativa de node-gtfs con timeout configurado
    ...
    if (error instanceof TypeError && error.message === 'fetch failed') {
      ...
      console.error('⏰ Timeout al actualizar los datos GTFS en tiempo real:', error.message);
    } else {
      console.error('❌ Error al actualizar los datos de GTFS:', error.message);
    }
