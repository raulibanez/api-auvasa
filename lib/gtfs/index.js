...
const { robustGtfsWrapper } = require('./robust-gtfs-wrapper');
...
const updateGtfsRt = async () => {
  try {
    // Intentar actualizar los datos GTFS en tiempo real usando wrapper robusto
    const result = await robustGtfsWrapper.updateGtfsRealtime(gtfs, gtfsConfig);
    
    // Si se aplicó fallback graceful, no procesar caché
    if (result && result.fallbackApplied) {
      console.log('⚠️  Usando datos cached debido a fallo en actualización realtime');
      return;
    }

    // Solo guardamos en caché los tiempos RT si está activado en la configuración
    if (GTFS_RT_CACHE === 'True') {
      ...
      for (let update of stopTimesUpdates) {
        ...
        if (checkCache(key)) {
          ...
          if (hasChanged) {
            console.log(`La clave ${key} existe en caché y ha cambiado. Actualizando...`);
            // Solo actualizar el caché si el nuevo valor no está vacío
            if (update && Object.keys(update).length > 0) {
              storeInCache(key, update);
              hasUpdates = true;
            } else {
              console.log(`La clave ${key} vino vacía, dejamos la existente en caché`);
            }
          }
        } else {
          ...
          if (update && Object.keys(update).length > 0) {
            ...
            console.log(`La clave ${key} vino vacía, no se añade a la caché`);
          }
        }
      }
    ...
    if (error instanceof TypeError && error.message === 'fetch failed') {
      console.error('Error de red al actualizar los datos GTFS en tiempo real:', error.cause);
    } else {
      console.error('Error al actualizar los datos de GTFS:', error);
    }
    
    // No lanzamos el error, permitimos que la aplicación continúe
    console.log('Continuando con la ejecución a pesar del error...');
  }
};
...
const getRTStopTimes = async (stopTimes, route) => {
  ...
  // OPTIMIZACIÓN: Obtener todas las posiciones de vehículos de una vez (batch query)
  const uniqueTripIds = [...new Set(stopTimes.map(sT => sT.trip_id))];
  const allVehiclePositions = await Promise.all(
    uniqueTripIds.map(async (tripId) => {
      try {
        const positions = await gtfs.getVehiclePositions({ trip_id: tripId });
        return { tripId, positions };
      } catch (error) {
        console.warn(`Error getting vehicle position for trip ${tripId}:`, error);
        return { tripId, positions: [] };
      }
    })
  );

  // Crear mapa para lookup O(1) de posiciones de vehículos
  const vehiclePositionsMap = new Map();
  allVehiclePositions.forEach(({ tripId, positions }) => {
    if (positions && positions.length > 0) {
      // Transformar al formato esperado por el código existente
      ...
      vehiclePositionsMap.set(tripId, formattedPositions);
    }
  });
  ...
  const rtStopTimesNew = Array.from(stopTimesMap).map(
    async ([key, scheduledStop]) => {
      ...
      // OPTIMIZACIÓN: Usar lookup O(1) en lugar de consulta individual
      const vehiclePosition = vehiclePositionsMap.get(scheduledStop.trip_id) || [];
...
const gtfsGetBusPosition = async (tripId) => {
  try {
    ...
    console.error(
      'Error al obtener las posiciones de los vehículos para el viaje %s:',
      tripId,
      error
    );
